export default async function updateDocument(request, response) {
  try {
    ...
    if (token !== undefined) {
      ...
      const allowedKeys = ['name', 'note', 'description', 'folderId','clarifications'];
      ...
      if (isValid) {
        ...
        if (res) {
          ...
          if (isArchive && isArchive) {
           ...
           // Update clarifications Array, Ajay
           if(request?.body?.clarification){

              updateQuery.addUnique('clarification', request?.body?.clarification);
