...
async function saveUser(userDetails) {
  ...
  console.log('>>> Received Request for user sign up', userDetails.email);
  console.log('>>> Query Returned '+userDetails.email,userRes);
  ...
  if (userRes) {
    ...
    console.log(' <<<< Login Response Session Token >>>> '+userDetails.email,login.sessionToken);
    return { id: login.objectId, sessionToken: login.sessionToken };
  } else {
    console.log('>>> Else Block saveUser usersignup '+userDetails.email);
    const user = new Parse.User();
    user.set('username', userDetails.email);
    user.set('password', "Pass@123");
    ...
    var sessionAccess = res.getSessionToken();
    if(!res.getSessionToken()){

      const loggedInUser = await Parse.User.logIn(userDetails.email, "Pass@123");
      sessionAccess = loggedInUser.getSessionToken();
    ...
    console.log('>>> After Sign up res object '+userDetails.email,sessionAccess);
    // console.log("res ", res);
    return { id: res.id, sessionToken: sessionAccess };
  }
}
